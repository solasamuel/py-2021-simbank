...
class Front:
    ...
    def login(self):
        ...
        self.master.title('SimBank')
        self.header = ttk.Frame(self.master)
        self.menu_count = 0

        self.logo = PhotoImage(file='images/simBankLogo.png').subsample(5, 5)
        self.header.pack()

        ttk.Label(self.header, image=self.logo).grid(
            row=0, column=0, rowspan=1)
        ttk.Label(self.header, text='Welcome to SimBank!').grid(
            row=0, column=1, columnspan=2)
        
        self.body = ttk.Frame(self.master)
        self.body.pack()
        self.body.config(relief=RIDGE, padding=(30, 15))

        ttk.Label(self.body, text='Username:').grid(
            row=0, column=1, padx=10, pady=10, sticky='sw')
        ttk.Label(self.body, text='Password:').grid(
            row=1, column=1, padx=10, pady=10, sticky='sw')
            
        self.user = ttk.Entry(self.body, width=24, font=('Arial', 10))
        self.psd = ttk.Entry(self.body, width=24, font=('Arial', 10))
        ...
        def temp_action():
            ...
            if self.log_name == self.vname and self.log_pass == self.vpass:
                messagebox.showinfo(
                    title='Welcome to SimBank', message='Welcome {}'.format(self.log_name))
                self.home()
            else:
                messagebox.showerror(
                    title='Invalid', message='Please check your username or password')

        ttk.Button(self.body, text="Submit", command=temp_action).grid(
            row=2, column=0, padx=10, pady=10, columnspan=2)
        ttk.Button(self.body, text="Create account", command=self.create).grid(
            row=2, padx=10, pady=10, column=2, stick='se')
        ttk.Button(self.body, text="Forgot Password?", command=self.forgot).grid(
    ...
    def forgot(self):
        ...
        self.body.pack()
        self.body.config(relief=RIDGE, padding=(30, 15))
        ttk.Label(self.body, text='Enter email address:').grid(
            row=0, column=1, padx=10, pady=10, sticky='sw')
        self.otpemail = ttk.Entry(
            self.body, width=24, font=('Arial', 10))
        ...
        ttk.Button(self.body, text="Send OTP", command=on_click_email).grid(
            row=1, column=1, columnspan=2, padx=10, pady=10)
        ttk.Button(self.body, text="Cancel", command=self.already).grid(
    ...
    def otppage(self):
        ...
        ttk.Label(self.body, text='Enter otp: ').grid(
            row=0, column=1, padx=10, pady=10, sticky='sw')
        self.otpvalue = ttk.Entry(
            self.body, width=24, font=('Arial', 10))
        self.otpvalue.grid(row=0, column=2)
        ttk.Button(self.body, text="Enter", command=temp_otp_action).grid(
            row=1, column=1, columnspan=2, padx=10, pady=10)
        ttk.Button(self.body, text="Cancel", command=self.already).grid(
            row=2, column=1, columnspan=2, padx=10, pady=10)
    ...
    def change_pass_in_otp(self):
        ...
        ttk.Button(self.body, text='Save', command=temp_pass_action).grid(
    ...
    def menu_ini(self):
        ...
        self.menu.pack()
        self.menu.config(relief=RIDGE)
        self.homebut = ttk.Button(
            self.menu, text="Home", command=self.home)
        self.homebut.grid(row=0, column=0)
        self.probut = ttk.Button(
            self.menu, text="Profile", command=self.profile)
        self.probut.grid(row=0, column=1)
        self.fribut = ttk.Button(
            self.menu, text="Friends", command=self.friends)
        self.fribut.grid(row=0, column=2)
        self.tranbut = ttk.Button(
            self.menu, text="Transfer amount", command=self.transfer)
        self.tranbut.grid(row=0, column=3)
        self.seabut = ttk.Button(
            self.menu, text="Search", command=self.search)
    ...
    def home(self):
        ...
        if self.profile_photo:
            ...
            ttk.Label(self.body, image=self.newpython).grid(
                row=0, column=0)

        ttk.Button(self.body, text="Check Account Balance",
                    command=self.check_balance).grid(row=1, padx=30, pady=5, column=1)
        ttk.Button(self.body, text="View Recent Transactions",
                    command=self.transaction_history).grid(row=2, padx=30, pady=10, column=1)
        ttk.Button(self.body, text="Check Usage Graph",
                    command=self.graphpage).grid(row=3, padx=30, pady=10, column=1)
        ttk.Button(self.body, text="Log out", command=self.login).grid(
            row=4, padx=30, pady=10, column=1)
        ttk.Label(self.body, wraplength=300, text="Welcome to SimBank. This is a simple, fast way of transferring money to anyone swiftly. Explore for more features.").grid(
    ...
    def graphpage(self):
        ...
        if self.user_list:
            ...
            ttk.Button(self.body, text="Back", command=self.home).pack()
            ttk.Button(self.body, text="Clear Usage History",
            ...
            self.master.geometry('640x430+300+100')
            ttk.Label(self.body, text="You have made no Transactions yet ").grid(
                row=1, padx=5, pady=5, column=0)
            ttk.Button(self.body, text='Go-Back',
    ...
    def create(self):
        ...
        ttk.Label(self.body, text="Email: ").grid(
            row=1, padx=5, pady=5, column=0)
        ttk.Label(self.body, text="DOB: ").grid(
            row=2, padx=5, pady=5, column=0)
        ttk.Label(self.body, text="Mobile: ").grid(
            row=3, padx=5, pady=5, column=0)
        ttk.Label(self.body, text="Address: ").grid(
            row=4, padx=5, pady=5, column=0)
        ttk.Label(self.body, text="Enter a password: ").grid(
            row=5, padx=5, pady=5, column=0)
        ttk.Label(self.body, text="Re-enter password: ").grid(row=6,
                                                                    padx=5, pady=5, column=0)
        ttk.Label(self.body, text="Bank: ").grid(
            row=0, padx=5, pady=5, column=2)
        ttk.Label(self.body, text="Account Number: ").grid(
            row=1, padx=5, pady=5, column=2)
        ttk.Label(self.body, text="Enter a UPI key: ").grid(
            row=2, padx=5, pady=5, column=2)
        ttk.Label(self.body, text="Re-enter UPI key: ").grid(row=3,
                                                                    padx=5, pady=5, column=2)
        ttk.Label(self.body, text="Upload Profile Picture: ").grid(row=4,
                                                                            padx=5, pady=5, column=2)

        self.name = ttk.Entry(self.body, text="Name: ")
        self.name.grid(row=0, padx=5, pady=5, column=1)
        self.email = ttk.Entry(self.body, text="Email: ")
        self.email.grid(row=1, padx=5, pady=5, column=1)

        self.dob = DateEntry(self.body, width=18, background='lightblue', foreground='black',
                                borderwidth=2, year=2000, month=1, day=1, date_pattern='dd/MM/yyyy')
        self.dob.grid(row=2, padx=5, pady=5, column=1)

        self.phone = ttk.Entry(self.body, text="Phone: ")
        self.phone.grid(row=3, padx=5, pady=5, column=1)
        self.address = ttk.Entry(self.body, text="Address: ")
        self.address.grid(row=4, padx=5, pady=5, column=1)
        self.pswd = ttk.Entry(self.body, text="Enter a password: ")
        self.pswd.grid(row=5, padx=5, pady=5, column=1)
        self.pswd.config(show='*')
        self.repswd = ttk.Entry(self.body, text="Re-enter password: ")
        self.repswd.grid(row=6, padx=5, pady=5, column=1)
        self.repswd.config(show='*')

        self.bank = ttk.Combobox(self.body, width=18)
        self.bank.grid(row=0, padx=5, pady=5, column=3)
        self.bank.set('Select your bank')
        self.bankoptions = ['SBG', 'Musk Bank', 'IBQ', 'Relay Bank']
        self.bank.config(values=self.bankoptions)

        self.acc = ttk.Entry(self.body, text="Account Number: ")
        self.acc.grid(row=1, padx=5, pady=5, column=3)
        self.upi = ttk.Entry(self.body, text="Enter a UPI key: ")
        self.upi.grid(row=2, padx=5, pady=5, column=3)
        self.upi.config(show='*')
        self.reupi = ttk.Entry(self.body, text="Re-enter UPI key: ")
        self.reupi.grid(row=3, padx=5, pady=5, column=3)
        self.reupi.config(show='*')
        button_explore = ttk.Button(self.body,
                                    text="Upload Profile Picture",
                                    command=self.create_photo)
        button_explore.grid(column=3, row=4)

        ttk.Button(self.body, text='Create', command=self.temp_action2).grid(
            row=7, padx=5, pady=5, column=1, sticky='ne')
        ttk.Button(self.body, text='Cancel', command=self.edit_cancel_create).grid(
            row=7, padx=5, pady=5, column=2, sticky='nw')
        ttk.Label(self.body, text='Already have an account?').grid(
            row=8, padx=5, pady=5, column=1, columnspan=2)
        ttk.Button(self.body, text='Sign-in',
    ...
    def temp_action2(self):
        ...
        if not pattern:
            ...
            if (self.obj.insert_record(self.vname, self.vemail, self.vdob, self.vphone, self.vaddress, self.vfirstpass, self.vupi, self.vbank, self.vacc)) == 1:
                ...
                messagebox.showinfo(
                    title='Welcome to SimBank', message='Welcome {}'.format(self.vname))
                self.uid, self.vname, self.vemail, self.vaddress, self.vdob, self.vphone, self.vpass, self.vupi, self.vrecent, self.profile_photo, self.ouid, self.vbank, self.vacc, self.vbal = self.obj.get_details(
                    self.vname)
                self.profile_photo = self.obj.insert_blob(self.imgobj)
                self.home()
    ...
    def edit(self):
        ...
        ttk.Button(self.body, text='Back', command=self.profile).grid(
            row=6, padx=10, pady=10, column=0, sticky='nw')
        if self.profile_photo:
            with open('current.jpg', 'wb') as file:
                file.write(self.profile_photo)
    ...
    def psd_change(self):
        ...
        self.body.pack()
        self.body.config(relief=RIDGE, padding=(30, 15))
        ttk.Label(self.body, text='Enter Current password: ').grid(
            row=0, padx=10, pady=10, column=0)
        self.check_pass = ttk.Entry(self.body)
        self.check_pass.grid(row=0, column=1)
        self.check_pass.config(show='*')
        self.password_change()
        ttk.Button(self.body, text='Save', command=self.save_password).grid(
            row=3, padx=10, pady=10, column=0, columnspan=2)
        ttk.Button(self.body, text='Back', command=self.edit).grid(
    ...
    def transaction_history_friend(self, value):
        ...
        self.body.pack_forget()
        self.body = ttk.Frame(self.master)
        up_body = ttk.Frame(self.body)
        up_body.pack()
        up_body.config(relief=RIDGE, padding=(30, 15))
        canvas = Canvas(self.body)
        self.scroll = ttk.Scrollbar(
            self.body, orient=VERTICAL, command=canvas.yview)
        ...
        if self.friend_obj:
            ...
            ttk.Label(up_body, text=value).grid(
                row=0, column=0)
        ttk.Label(up_body, text="Recent Transactions", font="Helvetica 16 bold").grid(
        ...
        for j, i in enumerate(self.vals):
            if str(i[0]) == str(self.vacc) and str(i[1]) == str(facc):
                self.message = f'Sent to {i[5]} Rs. {i[2]} on {i[3][:19]}'
                ttk.Label(self.scrollable, text=self.message, foreground="red",
                            font="Helvetica 12 bold").grid(row=j+1, column=0,  pady=10, sticky='w')
            elif str(i[1]) == str(self.vacc) and str(i[0]) == str(facc):
                self.message = f'Received From {i[4]} Rs. {i[2]} on {i[3]}'
                ttk.Label(self.scrollable, text=self.message,
                            foreground="green",
                            font="Helvetica 12 bold").grid(row=j+1, column=0, pady=10, sticky='e')
        self.body.pack()
        self.body.config(relief=RIDGE, padding=(30, 15))
    ...
    def friends(self):
        def pay_friend(value):
            ...
            self.body.pack()
            self.body.config(relief=RIDGE, padding=(30, 15))
            ttk.Label(self.body, text="Enter the Amount").grid(
                row=0, padx=10, pady=10, column=0)
            self.money_tran_friend = ttk.Entry(self.body)
            self.money_tran_friend.grid(row=0, padx=10, pady=10, column=1)
            ttk.Button(self.body, text='Pay', command=lambda: self.pay_tran_friend(
                value)).grid(row=2, padx=10, pady=10, column=0, sticky='ne')
            ttk.Button(self.body, text='Cancel', command=self.edit_cancel2).grid(
                row=2, padx=10, pady=10, column=1)

        def pay_friend_interm(value):
            self.transaction_history_friend(value)
            ttk.Button(self.body, text='Pay',
                        command=lambda: pay_friend(value)).pack()#callback to a local function
            ttk.Button(self.body, text='Back',
        ...
        self.vals = self.obj.transaction_history(self.vacc)
        ttk.Label(up_body, text="Friends", font="Helvetica 16 bold").grid(
            row=0, column=0, padx=10, pady=1)
        canvas.create_window((0, 0), window=scrollable, anchor="nw")
        canvas.configure(yscrollcommand=scroll.set)
        self.friend_set = set()
        for j, i in enumerate(self.vals):
            if str(i[0]) == str(self.vacc) or str(i[1]) == str(self.vacc):
    ...
    def pay_tran_friend(self, value):
        self.text_for_check()
        ttk.Button(self.body, text='OK', command=lambda: self.after_pay_tran_friend(
    ...
    def text_for_check(self):
        ...
        self.body.pack()
        self.body.config(relief=RIDGE, padding=(30, 15))
        ttk.Label(self.body, text="Enter your UPI key: ").grid(
            row=0, padx=10, pady=10, column=0)
        self.check_upi = ttk.Entry(self.body)
        self.check_upi.grid(row=0, padx=10, pady=10, column=1)
        self.check_upi.config(show='*')

    def check_balance(self):
        self.text_for_check()
        ttk.Button(self.body, text='OK', command=self.check_balance2).grid(
    ...
    def check_balance2(self):
        ...
        if self.hereamount == -1:
            ...
            ttk.Label(self.body, text="Your Account Balance is:{} ".format(
                self.hereamount)).grid(row=0, padx=10, pady=10, column=0)
            ttk.Button(self.body, text='Back', command=self.home).grid(
                row=1, padx=10, pady=10, column=0)
    ...
    def transaction_history(self):
        ...
        canvas.configure(yscrollcommand=self.scroll.set)
        for j, i in enumerate(self.vals):
            if str(i[0]) == str(self.vacc):
                self.message = f'Sent to {i[5]} Rs. {i[2]} on {i[3][:19]}'
                ttk.Label(self.scrollable, text=self.message, foreground="red",
                            font="Helvetica 12 bold").grid(row=j+1, column=0,  pady=10, sticky='w')
            elif str(i[1]) == str(self.vacc):
                self.message = f'Received From {i[4]} Rs. {i[2]} on {i[3]}'
                ttk.Label(self.scrollable, text=self.message,
                            foreground="green",
                            font="Helvetica 12 bold").grid(row=j+1, column=0, pady=10, sticky='e')
    ...
    def transfer(self):
        ...
        self.body.pack()
        self.body.config(relief=RIDGE, padding=(30, 15))
        ttk.Label(self.body, text="Enter the Account Number").grid(
            row=0, padx=10, pady=10, column=0)
        ttk.Label(self.body, text="Enter the Amount").grid(
            row=1, padx=10, pady=10, column=0)
        self.acc_tran = ttk.Entry(self.body)
        self.acc_tran.grid(row=0, padx=10, pady=10, column=1)
        self.money_tran = ttk.Entry(self.body)
        self.money_tran.grid(row=1, padx=10, pady=10, column=1)
        ttk.Button(self.body, text='Pay', command=self.pay_tran).grid(
    ...
    def pay_tran(self):
        self.text_for_check()
        ttk.Button(self.body, text='OK', command=self.after_pay_tran).grid(
            row=1, padx=10, pady=10, column=1)
    ...
    def search(self):
        def pay_friend(value):
            ...
            ttk.Button(self.body, text='Pay', command=lambda: self.pay_tran_friend(
                value)).grid(row=2, padx=10, pady=10, column=0, sticky='ne')
            ttk.Button(self.body, text='Cancel', command=self.search).grid(
                row=2, padx=10, pady=10, column=1)
        ...
        def search_list():
            ...
            if resultname != self.vname:
                ttk.Button(self.body, text=resultname, command=lambda: pay_friend_interm(
        ...
        ttk.Label(self.body, textvariable=self.search_var).grid(
            row=1, padx=10, pady=10, column=0)
        search_combo = ttk.Combobox(
            self.body, textvariable=self.search_var)
