...
class OneUser:
    def __init__(self):
        self.conn = sqlite3.connect('db-simBank.db')
    ...
    def get_details(self, name):
        ...
        self.curs.execute('SELECT * FROM bankAcc where id = ?', [self.id])
    ...
    def insert_record(self, *create_details):
        ...
        try:
            self.curs.execute(f'SELECT id FROM bankAcc WHERE account = {acc_no}')
    ...
    def check_balance(self, uid, upikey):
        try:
            if self.upi != int(upikey):
                return -1
            self.curs.execute(
                f"SELECT balance FROM bankAcc WHERE id = {self.id} ")
    ...
    def pay(self, accno, accmoney):
        ...
        self.curs.execute(f"SELECT balance FROM bankAcc WHERE id = ?", [self.id])
        tbal = self.curs.fetchone()[0]
        self.curs.execute(f"SELECT balance FROM bankAcc WHERE account = {accno}")
        rbal = int(self.curs.fetchone()[0])
        sender = self.name
        self.curs.execute(f"SELECT id from bankAcc WHERE account = ?", [accno])
        ...
        self.curs.execute(
            f"SELECT balance from bankAcc WHERE id = ?", [receiverid])
        ...
        try:
            self.curs.execute(
                f"UPDATE bankAcc SET balance = {rbal + accmoney} WHERE account = '{accno}'")
            self.curs.execute(
                f"UPDATE bankAcc SET balance = {tbal - accmoney} WHERE account = '{self.account}'")
            if tbal-accmoney < 0:
                return 'lowbal'

            self.curs.execute(f"INSERT INTO transactions VALUES (?,?,?,?,?,?)", [
    ...
    def transaction_history(self, uid):
        self.curs.execute(
            f"SELECT * FROM transactions WHERE fromAcc = ? OR toAcc = ?", [uid, uid])
    ...
    def pay_friend(self, uid, sname, fname, fmoney):
        ...
        self.curs.execute(
            f"SELECT balance,account from bankAcc WHERE id = ?", [friendid])
        friendmon, friendacc = self.curs.fetchone()
        self.curs.execute(
            f"SELECT balance,account from bankAcc WHERE id = ?", [uid])
        ...
        if sendermon-fmoney < 0:
            return 'lowbal'
        self.curs.execute(
            f"UPDATE bankAcc SET balance = {friendmon + fmoney} WHERE id = '{friendid}'")
        self.curs.execute(
            f"UPDATE bankAcc SET balance = {sendermon - fmoney} WHERE id = '{uid}'")
    ...
    def friend_info(self, value):
        ...
        self.curs.execute(f"SELECT account FROM bankAcc WHERE id = ?", [id])
    ...
    def send_otp(self, email):
        try:
            ...
            message = f"Your OTP for SimBank is {str(self.randnumber)}. If not done by you, please report immediately."
            print(message)
